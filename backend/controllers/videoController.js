...
exports.getPurchasedVideos = async (req, res) => {
  try {
    console.log("Fetching purchased videos for user:", req.user.id);
    ...
    // Filter out any missing or null videos
    const videos = purchases
      .map(p => p.video)
      .filter(v => v !== null);
    ...
    res.status(500).json({ msg: "Failed to fetch purchased videos", error: err.message });
  }
};
